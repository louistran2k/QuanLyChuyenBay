...
void NhapNgay(Date &date) {
    do {
        ...
        if (!NgayHopLe(date))cout << "ngay thang nam k hop le,yeu cau nhap lai: ";
    } while (!NgayHopLe(date));
}
void NhapGio(Hour& h) {
    do {
        cin >> h.gio >> h.phut;
        if (!GioHopLe(h))cout << "Gio phut k hop le,yeu cau nhap lai: ";
    } while (!GioHopLe);
}
Date NgayHienTai() {
    Date date;
...
{	
	...
	else if(d1.nam=d_ht.nam)
	{
		...
		else if(d1.thang=d_ht.thang)
		{
			...
			else if(d1.ngay>=d_ht.ngay) return true;
		}
	}
}
